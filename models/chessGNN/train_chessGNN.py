...
from models.chessGNN.chess_graph_dataset import ChessGraphDataset
from models.chessGNN.model import Model
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=os.environ.get("CHECKPOINT_ARTIFACT_PATH"),
    )
    parser.add_argument(
        "--load-path",
    ...
    parser.add_argument(
        "--only-load-model-and-optimizer",
        help="only load model parameters and optimizer state",
        type=int,
        default=0,
    )
    parser.add_argument("--bs", help="batch size", type=int, default=64)
    parser.add_argument("--lr", help="learning rate", type=float, default=0.0008)
    ...
    parser.add_argument(
        ...
        help="dataset id",
        type=str,
        default="c8a58a24-8312-4258-9878-85f31e636d93",
    )
    return parser
...
def main(args, timer):
    ...
    data_path = f"/data/{args.dataset}/chess_dataset_hdf5"
    dataset = ChessGraphDataset(data_path)
    ...
    if checkpoint_path:
        ...
        if (
            args.load_path
            and args.only_load_model_and_optimizer == 1
            and not os.path.islink(local_resume_path)
        ):
            ...
            timer.report("Retrieved saved checkpoint (special case)")
        else:
            checkpoint = torch.load(
                checkpoint_path, map_location=f"cuda:{args.device_id}"
            )
    ...
    for epoch in range(train_dataloader.sampler.epoch, 10_000):
        with train_dataloader.sampler.in_epoch(epoch):
            ...
            for data_batch in train_dataloader:
                ...
                data_batch = data_batch.to(args.device_id)
                ...
                loss = loss_fn(logits, data_batch.y) / args.grad_accum

                loss.backward()
                train_dataloader.sampler.advance(len(data_batch.y))

                # How accurately do our model scores rank the batch of moves?
                rank_corr = spearmans_rho(logits, data_batch.y)

                metrics["train"].update(
                    {
                        "examples_seen": len(data_batch.y),
...
Avg Loss [{avg_loss:,.3f}], Rank Corr.: [{rpt_rank_corr:,.3f}%], Examples: {rpt['examples_seen']:,.0f}"""
                    ...
                    if args.is_master:
                        total_progress = (
                            train_dataloader.sampler.progress
            ...
            with test_dataloader.sampler.in_epoch(epoch):
                ...
                with torch.no_grad():
                    for data_batch in test_dataloader:
                        ...
                        loss = loss_fn(logits, data_batch.y)
                        test_dataloader.sampler.advance(len(data_batch.y))
                        ...
                        if is_last_batch:
                            ...
                            if args.is_master:
                                ...
                                writer.add_scalar(
                                    "test/batch_rank_corr", rpt_rank_corr, epoch
                                )
