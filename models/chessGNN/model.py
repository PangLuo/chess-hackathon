"""
This module defines a PyTorch Geometric-based neural network model for evaluating chess positions.
It processes chess board states into graph representations and predicts a scalar evaluation score.

Key Components:
- `get_node_features(board)`: Extracts numerical features for each square on the chessboard.
- `get_edge_index_and_features(board)`: Constructs edges between squares based on chess piece interactions.
- `Model(nn.Module)`: A graph neural network using NNConv and GlobalAttention for position evaluation.

Utility Functions:
- `_is_attack(board, from_square, to_square)`: Determines if a move is an attack.
- `_is_defend(board, from_square, to_square)`: Determines if a move is a defensive action.

Usage:
The `Model` class can be used to evaluate chess moves by providing a PGN string and a move in SAN notation.
"""
